...
HW_resize(ImagePtr I1, int ww, int hh, int kernelID, double param, ImagePtr I2)
{
	...
	int t;
	ChannelPtr<uchar> src, dst, I3_chptr, I3_chptr_head;
	for (int ch = 0; IP_getChannel(II, ch, src, t); ch++) {
		
		IP_getChannel(I2, ch, dst, t);
		IP_getChannel(I3, ch, I3_chptr, t);
		I3_chptr_head = I3_chptr;
		// horizontal pass scales rows

		//for each row ( row 0 to row h - 1)
		for (int y = 0; y < h; ++y) {
			// PUT YOUR CODE HERE
			resize1D(src, w, ww, 1, kernelID, param, I3_chptr);
			src += w;
			I3_chptr += ww;

		}
		I3_chptr = I3_chptr_head;

		// vertical pass scales columns
		for (int x = 0; x < ww; ++x) {
			// PUT YOUR CODE HERE
			resize1D(I3_chptr, h, hh, ww, kernelID, param, dst);
			I3_chptr++;
			dst++;
